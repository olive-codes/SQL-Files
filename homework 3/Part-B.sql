...
--1
SELECT restaurant.name, MAX(stars) FROM restaurant, rating WHERE restaurant.rID = rating.rID GROUP BY restaurant.name ORDER BY restaurant.name ASC;
--2
SELECT restaurant.name, Max(stars) - MIN(stars) as ratingSpread FROM restaurant, rating WHERE restaurant.rID = rating.rID GROUP BY restaurant.name ORDER BY ratingSpread DESC, restaurant.name ASC;
--3
SELECT (MAX(rated) - MIN(rated)) as differenceInAvgRating FROM (SELECT AVG(rate) as rated FROM (SELECT AVG(stars) as rate FROM rating JOIN restaurant ON rating.rID = restaurant.rID WHERE restaurant.cuisine = 'Indian')
UNION SELECT AVG(rate) as rated FROM (SELECT AVG(stars) as rate FROM rating INNER JOIN restaurant ON rating.rID = restaurant.rID WHERE restaurant.cuisine = 'Chinese'));

--4 No reviewers reviewed both Chinese and Indian restaraunts

SELECT reviewer.vID as vID, reviewer.name as name FROM restaurant
JOIN rating ON restaurant.rID = rating.rID
JOIN reviewer ON reviewer.vID = rating.vID
WHERE cuisine = 'Indian' AND cuisine = 'Chinese';